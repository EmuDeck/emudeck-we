function game_mode_enable(){
	$source = "$userFolder\AppData\Roaming\EmuDeck\backend\tools\gamemode\start_game_mode.bat"
	...
	& "$env:APPDATA\EmuDeck\backend\tools\gamemode\enable.bat"
	if($?){
		Write-Output "true"
	}
}
...
function game_mode_disable(){
	...
	& "$env:APPDATA\EmuDeck\backend\tools\gamemode\disable.bat"
	if($?){
		Write-Output "true"
	}
}
		...
		if ($emuName -eq 'all'){
			...
			if (Test-Path -PathType Any "$target\.hash"){
				...
				if ($hash -eq $hashCloud){
					$toast = steamToast -MessageText "Saves up to date, no need to sync"
				}else{
					$toast = steamToast -MessageText "Downloading saves for all installed system, please wait..."
			...
			if (Test-Path -PathType Any "$target\.hash"){
				if ($hash -eq $hashCloud){
					...
					$toast = steamToast -MessageText "Downloading saves for $emuName, please wait..."

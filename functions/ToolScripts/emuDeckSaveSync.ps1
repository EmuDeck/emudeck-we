		...
		if ($emuName -eq 'all'){
			...
			if (Test-Path -PathType Any "$fileHash"){
				if ($hash -eq $hashCloud){
					...
					& $cloud_sync_bin copy --fast-list --update --tps-limit 12 --checkers=50 --exclude=/.fail_upload --exclude=/.fail_download --exclude=/system/prod.keys --exclude=/system/title.keys --exclude=/.pending_upload --exclude=/.watching --exclude=/*.lnk --exclude=/.cloud --exclude=/.emulator -q --log-file "$userFolder/EmuDeck/logs/rclone.log" --exclude=/.user "$cloud_sync_provider`:Emudeck\saves\$emuName\" "$target"
				}
			}else{
				$dialog = steamToast  -MessageText "Downloading saves for $emuName, please wait..."
				& $cloud_sync_bin copy --fast-list --update --tps-limit 12 --checkers=50 --exclude=/.fail_upload --exclude=/.fail_download --exclude=/system/prod.keys --exclude=/system/title.keys --exclude=/.pending_upload --exclude=/.watching --exclude=/*.lnk --exclude=/.cloud --exclude=/.emulator -q --log-file "$userFolder/EmuDeck/logs/rclone.log"  --exclude=/.user "$cloud_sync_provider`:Emudeck\saves\$emuName\" "$target"
...
function cloud_sync_upload{
	...
	if ((Test-Path "$cloud_sync_bin") -and ($cloud_sync_status -eq $true)) {
		...
		if ($emuName -eq 'all'){
			...
			& $cloud_sync_bin copy --fast-list --update --tps-limit 12 --checkers=50 --exclude=/.fail_upload --exclude=/.fail_download --exclude=/system/prod.keys --exclude=/system/title.keys --exclude=/.pending_upload --exclude=/.watching --exclude=/*.lnk --exclude=/.cloud --exclude=/.emulator --exclude=/.user -q --log-file "$userFolder/EmuDeck/logs/rclone.log" "$target" "$cloud_sync_provider`:Emudeck\saves\"
			if ($?) {
				$baseFolder = "$target"
				$timestamp = Get-Date -Format "yyyy-MM-dd HH:mm:ss"
				Get-ChildItem -Directory $baseFolder | ForEach-Object {
			...
			& $cloud_sync_bin copy -q --log-file "$userFolder/EmuDeck/logs/rclone.log" --fast-list --update --tps-limit 12 --checkers=50 --exclude=/.fail_upload --exclude=/.fail_download --exclude=/system/prod.keys --exclude=/system/title.keys --exclude=/.pending_upload --exclude=/.watching --exclude=/*.lnk --exclude=/.cloud --exclude=/.emulator --exclude=/.user "$target" "$cloud_sync_provider`:Emudeck\saves\$emuName\"
			if ($?) {

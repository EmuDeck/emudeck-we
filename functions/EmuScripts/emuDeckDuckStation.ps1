...
function DuckStation_init(){	
	...
	copyFromTo "$HOME\EmuDeck\backend\configs\DuckStation" $destination
...
function DuckStation_setupSaves(){
	...
	#Saves
	$SourceFilePath = -join($HOME, '\tools\EmulationStation-DE\Emulators\duckstation\memcards')
	...
	#States
	$SourceFilePath = -join($HOME, 'tools\EmulationStation-DE\Emulators\duckstation\savestates')
